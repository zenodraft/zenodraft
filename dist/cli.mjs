...
export const cli = () => {
    ...
    const show = (() => {
        ...
        cmd.command('details')
            ...
            try {
                const access_token = helpers_get_access_token_from_environment(zenodraft.opts().sandbox);
                const details = yield deposition_show_details(access_token, zenodraft.opts().sandbox, id, 'deposition', zenodraft.opts().verbose);
        ...
        cmd.command('draft')
            ...
            try {
                const access_token = helpers_get_access_token_from_environment(zenodraft.opts().sandbox);
                const draft_id = yield deposition_show_draft(access_token, zenodraft.opts().sandbox, collection_id, zenodraft.opts().verbose);
        ...
        cmd.command('files')
            ...
            try {
                const access_token = helpers_get_access_token_from_environment(zenodraft.opts().sandbox);
                const filenames = yield deposition_show_files(access_token, zenodraft.opts().sandbox, id, zenodraft.opts().verbose);
        ...
        cmd.command('latest')
            ...
            try {
                const access_token = helpers_get_access_token_from_environment(zenodraft.opts().sandbox);
                const latest_id = yield deposition_show_latest(access_token, zenodraft.opts().sandbox, collection_id, zenodraft.opts().verbose);
